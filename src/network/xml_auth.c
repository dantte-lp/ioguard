 ...
 */
static int extract_tag_content(const char *xml, const char *tag, char *out, size_t out_size)
{
 ...
 */
static int extract_attribute(const char *tag_start, const char *attr, char *out, size_t out_size)
{
...
int iog_xml_parse_auth_request(const char *xml, size_t len, iog_xml_auth_request_t *out)
{
    ...
    /* Top-level fields */
    extract_tag_content(xml, "device-id", out->device_id, sizeof(out->device_id));
    extract_tag_content(xml, "group-select", out->group_select, sizeof(out->group_select));
    extract_tag_content(xml, "session-token", out->session_token, sizeof(out->session_token));
    ...
    if (ver_tag != nullptr) {
        extract_tag_content(xml, "version", out->client_version, sizeof(out->client_version));
        extract_attribute(ver_tag, "who", out->platform_version, sizeof(out->platform_version));
    }
    ...
    if (auth_start != nullptr) {
        ...
        if (auth_end != nullptr) {
            ...
            if (extract_tag_content(auth_block, "username", out->username, sizeof(out->username)) ==
                0) {
                if (out->username[0] != '\0') {
            ...
            if (extract_tag_content(auth_block, "password", out->password, sizeof(out->password)) ==
                0) {
                if (out->password[0] != '\0') {
            ...
            if (extract_tag_content(auth_block, "otp", out->otp, sizeof(out->otp)) == 0) {
                if (out->otp[0] != '\0') {
                    out->has_otp = true;
                }
            }
                               ...
                               size_t *out_len)
{
    ...
    switch (resp->type) {
    case IOG_XML_RESP_CHALLENGE:
        xb_append(&xb, "<config-auth client=\"vpn\""
                       " type=\"auth-request\">\n");

        if (resp->banner[0] != '\0') {
            char esc[1024];
            xml_escape(resp->banner, esc, sizeof(esc));
            xb_append(&xb, "<banner>%s</banner>\n", esc);
        }

        xb_append(&xb, "<opaque is-for=\"sg\">\n"
                       "<tunnel-group>default</tunnel-group>\n"
                       "</opaque>\n");

        xb_append(&xb, "<auth id=\"main\">\n"
                       "<title>Login</title>\n"
        ...
        if (resp->group_count > 0) {
            xb_append(&xb, "<select name=\"group_list\" "
                           "label=\"GROUP:\">\n");
            for (uint32_t i = 0; i < resp->group_count && i < IOG_XML_MAX_GROUPS; i++) {
                char esc_name[128];
                char esc_label[256];
                xml_escape(resp->groups[i].name, esc_name, sizeof(esc_name));
                xml_escape(resp->groups[i].label, esc_label, sizeof(esc_label));
                xb_append(&xb,
                          "<option value=\"%s\">%s"
                          "</option>\n",
                          esc_name, esc_label);
        ...
        xb_append(&xb, "<input type=\"text\" name=\"username\""
                       " label=\"Username:\"/>\n"
    ...
    case IOG_XML_RESP_MFA_CHALLENGE:
        xb_append(&xb, "<config-auth client=\"vpn\""
                       " type=\"auth-request\">\n");
        ...
        if (resp->mfa_message[0] != '\0') {
            ...
            xb_append(&xb, "<message>%s</message>\n", esc);
        } else {
            xb_append(&xb, "<message>Enter verification code"
                           "</message>\n");
        }

        xb_append(&xb, "<form method=\"post\" action=\"/auth\">\n"
    ...
    case IOG_XML_RESP_SUCCESS:
        ...
        if (resp->session_token[0] != '\0') {
            ...
            xb_append(&xb, "<session-token>%s</session-token>\n", esc);
        }
