...
iog_dtls_ctx_t *iog_dtls_create(const iog_dtls_config_t *cfg)
{
    ...
    if (cfg->cert_file) {
        if (wolfSSL_CTX_use_certificate_file(ctx->ssl_ctx, cfg->cert_file, SSL_FILETYPE_PEM) !=
            SSL_SUCCESS) {
    ...
    if (cfg->key_file) {
        if (wolfSSL_CTX_use_PrivateKey_file(ctx->ssl_ctx, cfg->key_file, SSL_FILETYPE_PEM) !=
            SSL_SUCCESS) {
            wolfSSL_CTX_free(ctx->ssl_ctx);
            free(ctx);
            return nullptr;
