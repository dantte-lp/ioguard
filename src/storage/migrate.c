...
static const char *const DDL_SCHEMA_VERSION =
	"CREATE TABLE IF NOT EXISTS schema_version ("
	"  version    INTEGER NOT NULL,"
	"  applied_at TEXT DEFAULT (datetime('now'))"
	")";

static const char *const SQL_GET_VERSION =
	"SELECT MAX(version) FROM schema_version";

static const char *const SQL_INSERT_VERSION =
...
int rw_sqlite_migrate(rw_sqlite_ctx_t *ctx)
{
	...
	rc = sqlite3_exec(ctx->db, DDL_SCHEMA_VERSION, nullptr, nullptr,
	                   &errmsg);
	if (rc != SQLITE_OK) {
		sqlite3_free(errmsg);
		goto rollback;
	...
	rc = sqlite3_prepare_v2(ctx->db, SQL_INSERT_VERSION, -1, &stmt,
	                         nullptr);
	if (rc != SQLITE_OK)
		goto rollback;
...
int rw_mdbx_check_format(rw_mdbx_ctx_t *ctx)
{
	...
	MDBX_val key = {.iov_base = (void *)FORMAT_KEY,
	                .iov_len  = sizeof(FORMAT_KEY) - 1};
	MDBX_val data = {0};
