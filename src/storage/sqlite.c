...
static const char *const DDL_USERS =
...
static const char *const DDL_AUDIT_LOG =
...
static const char *const DDL_BAN_LIST =
...
static const char *const SQL_BAN_ADD =
	"INSERT OR REPLACE INTO ban_list (ip, reason, expires_at)"
...
int rw_sqlite_init(rw_sqlite_ctx_t *ctx, const char *path)
{
	...
	sqlite3_db_config(ctx->db, SQLITE_DBCONFIG_ENABLE_LOAD_EXTENSION, 0,
	                  nullptr);
	...
	err = err ? err : prepare_stmt(ctx->db, SQL_USER_LOOKUP,
	                               &ctx->stmt_user_lookup);
	err = err ? err : prepare_stmt(ctx->db, SQL_USER_CREATE,
	                               &ctx->stmt_user_create);
	err = err ? err : prepare_stmt(ctx->db, SQL_AUDIT_INSERT,
	                               &ctx->stmt_audit_insert);
	err = err ? err : prepare_stmt(ctx->db, SQL_AUDIT_QUERY,
	                               &ctx->stmt_audit_query);
	err = err ? err : prepare_stmt(ctx->db, SQL_BAN_CHECK,
	                               &ctx->stmt_ban_check);
	err = err ? err : prepare_stmt(ctx->db, SQL_BAN_ADD,
	                               &ctx->stmt_ban_add);
	if (err != 0)
		goto cleanup;
...
int rw_sqlite_user_lookup(rw_sqlite_ctx_t *ctx, const char *username,
                          rw_user_record_t *out)
{
	...
	safe_copy(out->username, sizeof(out->username),
	          (const char *)sqlite3_column_text(stmt, 0));
	safe_copy(out->password_hash, sizeof(out->password_hash),
	          (const char *)sqlite3_column_text(stmt, 1));
	safe_copy(out->groups, sizeof(out->groups),
...
int rw_sqlite_audit_query_by_username(rw_sqlite_ctx_t *ctx,
                                      const char *username,
                                      ...
                                      size_t max_entries, size_t *count)
{
	...
	*count = 0;
	...
	while ((rc = sqlite3_step(stmt)) == SQLITE_ROW && *count < max_entries) {
		...
		memset(e, 0, sizeof(*e));
		safe_copy(e->event_type, sizeof(e->event_type),
		          (const char *)sqlite3_column_text(stmt, 0));
		safe_copy(e->username, sizeof(e->username),
		          (const char *)sqlite3_column_text(stmt, 1));
		safe_copy(e->source_ip, sizeof(e->source_ip),
		          (const char *)sqlite3_column_text(stmt, 2));
		e->source_port = (uint16_t)sqlite3_column_int(stmt, 3);
		safe_copy(e->auth_method, sizeof(e->auth_method),
		          (const char *)sqlite3_column_text(stmt, 4));
		safe_copy(e->result, sizeof(e->result),
		          (const char *)sqlite3_column_text(stmt, 5));
		safe_copy(e->details, sizeof(e->details),
		          (const char *)sqlite3_column_text(stmt, 6));
		safe_copy(e->session_id, sizeof(e->session_id),
