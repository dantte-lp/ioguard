...
constexpr size_t RW_VAULT_KEY_SIZE = 32;

/** AES-GCM IV size in bytes (96-bit recommended). */
constexpr size_t RW_VAULT_IV_SIZE = 12;

/** AES-GCM authentication tag size. */
constexpr size_t RW_VAULT_TAG_SIZE = 16;

/** Overhead per encrypted field: IV + tag. */
constexpr size_t RW_VAULT_OVERHEAD = RW_VAULT_IV_SIZE + RW_VAULT_TAG_SIZE;

/** Opaque vault context. */
typedef struct rw_vault rw_vault_t;
 ...
 */
[[nodiscard]] int rw_vault_init(const char *key_path, rw_vault_t **out);

/**
 * @brief Initialize vault from a raw key (for testing).
 *
 * @param key      32-byte master key.
 * @param key_len  Must be RW_VAULT_KEY_SIZE.
 * @param out      Output vault handle.
 ...
 * Total output: plain_len + RW_VAULT_OVERHEAD.
 *
 * @param vault     Vault context.
 * @param plaintext Input data.
 * @param plain_len Input length.
 * @param out       Output buffer (must be >= plain_len + RW_VAULT_OVERHEAD).
 ...
 * @param out        Output buffer (must be >= blob_len - RW_VAULT_OVERHEAD).
 * @param out_size   Output buffer capacity.
