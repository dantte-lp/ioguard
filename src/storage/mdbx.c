...
static int hsr_callback(const MDBX_env *env, const MDBX_txn *txn, mdbx_pid_t pid, mdbx_tid_t tid,
                        uint64_t laggard, unsigned gap, size_t space, int retry)
{
    (void)env;
    (void)txn;
    (void)tid;
...
int iog_mdbx_init(iog_mdbx_ctx_t *ctx, const char *path)
{
    ...
    MDBX_env_flags_t flags = MDBX_NOSUBDIR | MDBX_SAFE_NOSYNC | MDBX_LIFORECLAIM;
