...
constexpr size_t RW_CERT_PATH_MAX = 256;

/** Maximum OID string length for MS AD certificate template. */
constexpr size_t RW_CERT_TEMPLATE_OID_MAX = 128;

/** Maximum template name length. */
constexpr size_t RW_CERT_TEMPLATE_NAME_MAX = 64;

/** Maximum username field specifier length. */
constexpr size_t RW_CERT_USERNAME_FIELD_MAX = 32;

/** Maximum extracted username length. */
constexpr size_t RW_CERT_USERNAME_MAX = 256;

/** Certificate authentication configuration. */
typedef struct {
    char ca_cert_path[RW_CERT_PATH_MAX];       /**< Trusted CA for client certs */
    char crl_path[RW_CERT_PATH_MAX];           /**< CRL file path (optional) */
    bool require_eku;                           /**< Require EKU: Client Authentication */
    char template_oid[RW_CERT_TEMPLATE_OID_MAX]; /**< MS AD template OID (optional) */
    char template_name[RW_CERT_TEMPLATE_NAME_MAX]; /**< Required template name (optional) */
    char username_field[RW_CERT_USERNAME_FIELD_MAX]; /**< "CN", "SAN:email", "SAN:UPN" */
 ...
 */
[[nodiscard]] int rw_cert_auth_init(const rw_cert_auth_config_t *config);
 ...
 */
[[nodiscard]] int rw_cert_extract_username(const uint8_t *der, size_t der_len, const char *field,
                                           char *out, size_t out_size);

#endif /* RINGWALL_AUTH_CERT_H */
