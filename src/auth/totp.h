...
constexpr uint32_t RW_TOTP_TIME_STEP = 30;

/** Number of TOTP digits in the output code. */
constexpr uint32_t RW_TOTP_DIGITS = 6;

/** Raw TOTP secret size in bytes (160-bit SHA-1 key). */
constexpr size_t RW_TOTP_SECRET_SIZE = 20;

/** Maximum Base32-encoded secret length (including NUL). */
constexpr size_t RW_TOTP_SECRET_B32_MAX = 64;
 ...
 */
[[nodiscard]] ssize_t rw_base32_decode(const char *encoded, uint8_t *out, size_t out_size);

/**
 * Generate a TOTP code for the given secret and time.
 *
 * @param secret     Raw secret bytes (RW_TOTP_SECRET_SIZE).
 * @param secret_len Length of the secret in bytes.
 * @param time_step  Time counter value (unix_time / RW_TOTP_TIME_STEP).
 * @param code_out   Pointer to store the generated numeric code.
 * @return 0 on success, negative errno on failure.
 */
[[nodiscard]] int rw_totp_generate(const uint8_t *secret, size_t secret_len, uint64_t time_step,
                                   uint32_t *code_out);
 ...
 */
[[nodiscard]] int rw_totp_validate(const uint8_t *secret, size_t secret_len, uint32_t code,
                                   uint64_t time_now, uint32_t window);

/**
 * Generate a random TOTP secret.
 *
 * @param secret     Output buffer for the raw secret (RW_TOTP_SECRET_SIZE bytes).
 * @param secret_len Size of the output buffer.
 * @return 0 on success, negative errno on failure.
 */
[[nodiscard]] int rw_totp_generate_secret(uint8_t *secret, size_t secret_len);
 ...
 */
[[nodiscard]] ssize_t rw_totp_build_uri(const uint8_t *secret, size_t secret_len,
