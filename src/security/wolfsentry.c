...
int rw_wolfsentry_init(rw_wolfsentry_ctx_t *ctx)
{
	...
	wolfsentry_errcode_t ret = wolfsentry_init(
...
int rw_wolfsentry_load_json(rw_wolfsentry_ctx_t *ctx,
			    const char *json, size_t json_len)
{
	...
	wolfsentry_errcode_t ret = wolfsentry_config_json_oneshot(
		WOLFSENTRY_CONTEXT_ARGS_OUT_EX4(ctx->ws_ctx, thread),
...
rw_ws_result_t rw_wolfsentry_check_connection(
	...
	int protocol)
{
	...
	wolfsentry_errcode_t ret = wolfsentry_route_event_dispatch(
		WOLFSENTRY_CONTEXT_ARGS_OUT_EX4(ctx->ws_ctx, thread),
