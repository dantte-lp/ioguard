...
static const int authmod_extra_syscalls[] = {
    SCMP_SYS(flock),          SCMP_SYS(pread64), SCMP_SYS(pwrite64), SCMP_SYS(fdatasync),
    SCMP_SYS(socket),         SCMP_SYS(sendmsg), SCMP_SYS(recvmsg),  SCMP_SYS(connect),
    SCMP_SYS(getpid),         SCMP_SYS(gettid),  SCMP_SYS(tgkill),   SCMP_SYS(rt_sigaction),
    SCMP_SYS(rt_sigprocmask), SCMP_SYS(pipe2),
};
