...
rg -n "RINGWALL_IPC_FDPASS_H|RINGWALL_CORE_PROCESS_H|RINGWALL_CORE_MAIN_H|\bRW_FDPASS_MAX_FDS\b|\brw_fdpass_(send|recv)\b|\brw_process_(spawn|wait|signal|cleanup)\b|\brw_process_t\b|\brw_main_(parse_args|create_ipc_pair|create_accept_pair|create_signalfd)\b|\brw_fdpass\b|\brw_core\b" \
