...
description: Use when writing ANY C code in wolfguard — enforces file structure, naming, comments, error handling, memory, includes, tests. MANDATORY for all new and modified code.
---

# wolfguard Coding Standards
...
```c
#ifndef WOLFGUARD_MODULE_FILE_H
#define WOLFGUARD_MODULE_FILE_H
...
#endif /* WOLFGUARD_MODULE_FILE_H */
```

Rules:
- Include guard: `WOLFGUARD_MODULE_FILE_H` (e.g., `WOLFGUARD_CORE_SESSION_H`)
...
|---------|-----------|---------|
| Functions (public) | `wg_module_verb_noun` | `wg_session_create`, `wg_io_prep_recv` |
...
| Types (struct/enum) | `wg_module_name_t` | `wg_session_t`, `wg_io_ctx_t` |
| Enum values | `WG_MODULE_VALUE` | `WG_IPC_MSG_AUTH_REQUEST` |
| Constants (constexpr) | `WG_MODULE_NAME` | `WG_SESSION_COOKIE_SIZE` |
| Macros | `WG_MODULE_NAME` | `WG_HTTP_MAX_HEADERS` |
...
| Callback types | `wg_module_cb` or `_func_t` | `wg_io_cb`, `tls_push_func_t` |
 ...
 * @param cookie Cookie bytes (must be WG_SESSION_COOKIE_SIZE)
...
| Custom enum | `WG_*_SUCCESS = 0` | negative enum values |

### Error propagation pattern

```c
[[nodiscard]] int wg_foo_create(wg_foo_t **out)
{
    wg_foo_t *foo = calloc(1, sizeof(*foo));
    if (foo == nullptr) {
        return -ENOMEM;
    }

    int ret = wg_bar_init(&foo->bar);
    if (ret < 0) {
        goto cleanup;
    }

    ret = wg_baz_init(&foo->baz);
    if (ret < 0) {
        goto cleanup;
    }
...
wg_foo_t *foo = calloc(1, sizeof(wg_foo_t));
```
...
void wg_foo_destroy(wg_foo_t *foo)
{
    if (foo == nullptr) {
        return;
    }
...
explicit_bzero(session->cookie, WG_SESSION_COOKIE_SIZE);
...
| `[[nodiscard]]` | All functions returning error/pointer | `[[nodiscard]] int wg_init(void);` |
| `constexpr` | Compile-time constants (replaces `#define` for values) | `constexpr size_t WG_MAX = 1024;` |
...
| Empty initializer `{}` | Zero-init structs | `wg_foo_t foo = {};` |
...
| `_BitInt(N)` | Not needed in wolfguard — standard `uint*_t` sufficient |
...
{
    /* Arrange */
    wg_foo_t *foo = wg_foo_create();
    TEST_ASSERT_NOT_NULL(foo);

    /* Act */
    int ret = wg_foo_action(foo);
...
- `test_wg_mem_init_returns_zero`
- `test_wg_session_validate_invalid_returns_enoent`
- `test_wg_io_init_zero_depth_returns_null`
...
wg_session_create(store, user, group, 300, &session);

/* CORRECT: [[nodiscard]] forces check */
int ret = wg_session_create(store, user, group, 300, &session);
if (ret < 0) { return ret; }

/* WRONG: sizeof(type) */
malloc(sizeof(wg_session_t));
...
/* CORRECT: constexpr */
constexpr uint32_t WG_MAX_SESSIONS = 1024;
```
