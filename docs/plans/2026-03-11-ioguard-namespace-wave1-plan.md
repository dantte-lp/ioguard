...
Start the code namespace migration from legacy `iog_` / `IOG_` / `IOGUARD_` to:
...
- Do not rename every `iog_*` symbol in one pass.
...
2. Rename low-risk product constants from `IOG_*` / `IOGUARD_*` to `IOG_*` / `IOGUARD_*` where isolated.
...
- CMake target names like `iog_io`, `iog_crypto`, `iog_http`
- function/type prefixes across the full source tree
- protobuf artifact names like `iog_ipc.proto`
