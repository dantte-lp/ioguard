...
Create all source directories and empty stub files with proper include guards. Each `.h` file gets the include guard pattern `WOLFGUARD_MODULE_FILE_H` and each `.c` file gets just its own header include. No implementation yet — just the skeleton.
...
add_library(wg_ipc STATIC src/ipc/transport.c src/ipc/messages.c)
target_link_libraries(wg_ipc PUBLIC ${PROTOBUF_C_LIBRARIES} wg_memory)
target_include_directories(wg_ipc PUBLIC ${CMAKE_SOURCE_DIR}/src ${PROTOBUF_C_INCLUDE_DIRS})
```
...
if(BUILD_TESTING AND UNITY_INCLUDE_DIR AND UNITY_SRC_DIR)
    ...
    wg_add_test(test_ipc_transport tests/unit/test_ipc_transport.c wg_ipc wg_io)
endif()
```
...
`src/utils/memory.h`:
```c
#ifndef WOLFGUARD_UTILS_MEMORY_H
#define WOLFGUARD_UTILS_MEMORY_H

#include <stddef.h>

/* Initialize memory allocator (mimalloc if available) */
[[nodiscard]] int wg_mem_init(void);

/* Allocate memory. Returns nullptr for size == 0. */
[[nodiscard]] void *wg_mem_alloc(size_t size);

/* Allocate zeroed memory */
[[nodiscard]] void *wg_mem_calloc(size_t count, size_t size);

/* Reallocate memory */
[[nodiscard]] void *wg_mem_realloc(void *ptr, size_t new_size);
...
#endif /* WOLFGUARD_UTILS_MEMORY_H */
```
...
void test_wg_io_init_creates_context(void)
{
    wg_io_ctx_t *ctx = wg_io_init(64, 0);
    TEST_ASSERT_NOT_NULL(ctx);
    wg_io_destroy(ctx);
}

void test_wg_io_init_zero_depth_fails(void)
{
    wg_io_ctx_t *ctx = wg_io_init(0, 0);
    TEST_ASSERT_NULL(ctx);
}

void test_wg_io_run_once_with_timeout(void)
{
    wg_io_ctx_t *ctx = wg_io_init(64, 0);
    TEST_ASSERT_NOT_NULL(ctx);
    ...
    int fired = 0;
    int ret = wg_io_add_timeout(ctx, 10, &fired);
    TEST_ASSERT_EQUAL_INT(0, ret);

    ret = wg_io_run_once(ctx, 100); /* wait up to 100ms */
...
void test_wg_io_nop_completes(void)
{
    wg_io_ctx_t *ctx = wg_io_init(64, 0);
    TEST_ASSERT_NOT_NULL(ctx);

    int completed = 0;
    int ret = wg_io_submit_nop(ctx, &completed);
    TEST_ASSERT_EQUAL_INT(0, ret);

    ret = wg_io_run_once(ctx, 100);
    TEST_ASSERT_GREATER_OR_EQUAL_INT(0, ret);
...
{
    UNITY_BEGIN();
    RUN_TEST(test_wg_io_init_creates_context);
    RUN_TEST(test_wg_io_init_zero_depth_fails);
    RUN_TEST(test_wg_io_run_once_with_timeout);
    RUN_TEST(test_wg_io_nop_completes);
    return UNITY_END();
}
```
...
void test_wg_io_accept_and_recv(void)
{
    wg_io_ctx_t *ctx = wg_io_init(64, 0);
    TEST_ASSERT_NOT_NULL(ctx);
    ...
    char buf[64] = {0};
    int recv_done = 0;
    ret = wg_io_prep_recv(ctx, sv[1], buf, sizeof(buf), &recv_done);
    TEST_ASSERT_EQUAL_INT(0, ret);
...
void test_wg_io_send(void)
{
    ...
    int send_done = 0;
    ret = wg_io_prep_send(ctx, sv[0], msg, 5, &send_done);
    TEST_ASSERT_EQUAL_INT(0, ret);
...
void test_wg_io_signalfd(void)
{
    ...
    int sig_received = 0;
    int ret = wg_io_prep_read(ctx, sfd, &siginfo, sizeof(siginfo), &sig_received);
    TEST_ASSERT_EQUAL_INT(0, ret);
...
Also add to `main()`:
```c
RUN_TEST(test_wg_io_accept_and_recv);
RUN_TEST(test_wg_io_send);
RUN_TEST(test_wg_io_signalfd);
```

**Step 2: Run test to verify new tests fail**

Expected: Linker errors for `wg_io_prep_recv`, `wg_io_prep_send`, `wg_io_prep_read`
...
Implementation follows the same pattern as `wg_io_submit_nop`: allocate `wg_io_completion_t`, set callback that sets `*completed = 1`, prep the SQE, set data.
...
void test_wg_ipc_preserves_message_boundaries(void)
{
    ...
    wg_ipc_send(ch.parent_fd, msg1, sizeof(msg1));
    wg_ipc_send(ch.parent_fd, msg2, sizeof(msg2));
...
**Files:**
- Create: `src/ipc/proto/wg_ipc.proto`
- Create: `src/ipc/messages.h`
- Create: `src/ipc/messages.c`
...
Add to CMakeLists.txt before `wg_ipc` library:
```cmake
...
Update `wg_ipc` library to include generated sources:
```cmake
add_library(wg_ipc STATIC
    src/ipc/transport.c
    src/ipc/messages.c
    ${PROTO_GEN_DIR}/wg_ipc.pb-c.c
)
add_dependencies(wg_ipc proto_gen)
target_include_directories(wg_ipc PUBLIC
...
```bash
git add src/ipc/proto/wg_ipc.proto src/ipc/messages.h src/ipc/messages.c \
...
- Create: `tests/fixtures/wolfguard.toml` (test config file)

**Step 1: Create test fixture**

`tests/fixtures/wolfguard.toml`:
```toml
[server]
...
[tls]
cert-file = "/etc/wolfguard/server.pem"
key-file = "/etc/wolfguard/server.key"
min-version = "1.2"
...
seccomp = true
landlock = true
wolfsentry-config = "/etc/wolfguard/wolfsentry.json"
```
...
static const char *TEST_CONFIG = "tests/fixtures/wolfguard.toml";
...
void test_config_defaults_when_missing(void)
{
    ...
    const char *minimal = "tests/fixtures/wolfguard_minimal.toml";
...
void test_wg_process_spawn_and_wait(void)
{
    /* Spawn /bin/true — should exit 0 */
    wg_process_t proc;
    const char *argv[] = {"/bin/true", nullptr};
    int ret = wg_process_spawn(&proc, "/bin/true", argv);
    TEST_ASSERT_EQUAL_INT(0, ret);
    ...
    /* Wait for exit */
    int exit_status;
    ret = wg_process_wait(&proc, &exit_status, 5000);
    TEST_ASSERT_EQUAL_INT(0, ret);
    TEST_ASSERT_EQUAL_INT(0, exit_status);

    wg_process_cleanup(&proc);
}

void test_wg_process_spawn_exit_code(void)
{
    wg_process_t proc;
    const char *argv[] = {"/bin/false", nullptr};
    int ret = wg_process_spawn(&proc, "/bin/false", argv);
    TEST_ASSERT_EQUAL_INT(0, ret);
...
void test_wg_process_spawn_nonexistent(void)
{
    wg_process_t proc;
    const char *argv[] = {"/nonexistent", nullptr};
    int ret = wg_process_spawn(&proc, "/nonexistent", argv);
    /* Should fail at spawn or child exits immediately */
    if (ret == 0) {
        int exit_status;
        wg_process_wait(&proc, &exit_status, 5000);
        TEST_ASSERT_NOT_EQUAL(0, exit_status);
        wg_process_cleanup(&proc);
    } else {
...
void test_wg_process_kill(void)
{
    wg_process_t proc;
    const char *argv[] = {"/bin/sleep", "60", nullptr};
    int ret = wg_process_spawn(&proc, "/bin/sleep", argv);
    TEST_ASSERT_EQUAL_INT(0, ret);

    /* Send SIGTERM via pidfd */
    ret = wg_process_signal(&proc, SIGTERM);
    TEST_ASSERT_EQUAL_INT(0, ret);
...
{
    UNITY_BEGIN();
    RUN_TEST(test_wg_process_spawn_and_wait);
    RUN_TEST(test_wg_process_spawn_exit_code);
    RUN_TEST(test_wg_process_spawn_nonexistent);
    RUN_TEST(test_wg_process_kill);
    return UNITY_END();
}
```
...
void test_ipc_roundtrip_auth(void)
{
    ...
    if (pid == 0) {
        ...
        wg_ipc_auth_request_t req;
        if (wg_ipc_unpack_auth_request(buf, n, &req) != 0) { _exit(2); }
        ...
        ssize_t packed = wg_ipc_pack_auth_response(&resp, resp_buf, sizeof(resp_buf));
        if (packed <= 0) { _exit(3); }

        wg_ipc_send(ch.child_fd, resp_buf, packed);
        wg_ipc_free_auth_request(&req);
        close(ch.child_fd);
        _exit(0);
    }
    ...
    ret = wg_ipc_send(ch.parent_fd, buf, packed);
    TEST_ASSERT_EQUAL_INT(0, ret);

    /* Receive auth response */
    ssize_t n = wg_ipc_recv(ch.parent_fd, buf, sizeof(buf));
    TEST_ASSERT_GREATER_THAN(0, n);

    wg_ipc_auth_response_t resp;
    ret = wg_ipc_unpack_auth_response(buf, n, &resp);
    TEST_ASSERT_EQUAL_INT(0, ret);
