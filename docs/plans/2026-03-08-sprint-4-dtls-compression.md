...
constexpr size_t WG_COMPRESS_MAX_OUTPUT = 16384 + 256; /* headroom */
...
{
	wg_compress_ctx_t ctx;
	int ret = wg_compress_init(&ctx, WG_COMPRESS_NONE);
	TEST_ASSERT_EQUAL_INT(0, ret);
	TEST_ASSERT_EQUAL_UINT8(WG_COMPRESS_NONE, ctx.type);
	wg_compress_destroy(&ctx);
}

void test_compress_init_null(void)
{
	TEST_ASSERT_EQUAL_INT(-EINVAL, wg_compress_init(nullptr, WG_COMPRESS_NONE));
}

void test_compress_none_passthrough(void)
{
	wg_compress_ctx_t ctx;
	wg_compress_init(&ctx, WG_COMPRESS_NONE);

	const uint8_t data[] = { 0xDE, 0xAD, 0xBE, 0xEF };
	uint8_t out[64];

	int ret = wg_compress(&ctx, data, sizeof(data), out, sizeof(out));
...
void test_decompress_none_passthrough(void)
{
	...
	int ret = wg_decompress(&ctx, data, sizeof(data), out, sizeof(out));
...
void test_compress_input_too_large(void)
{
	...
	uint8_t out[64];
	int ret = wg_compress(&ctx, out, WG_COMPRESS_MAX_INPUT + 1, out, sizeof(out));
...
{
	TEST_ASSERT_EQUAL_STRING("none", wg_compress_type_name(WG_COMPRESS_NONE));
	TEST_ASSERT_EQUAL_STRING("lz4", wg_compress_type_name(WG_COMPRESS_LZ4));
	TEST_ASSERT_EQUAL_STRING("lzs", wg_compress_type_name(WG_COMPRESS_LZS));
}

void test_compress_negotiate(void)
{
	TEST_ASSERT_EQUAL_UINT8(WG_COMPRESS_LZS, wg_compress_negotiate("lzs,deflate"));
	TEST_ASSERT_EQUAL_UINT8(WG_COMPRESS_LZ4, wg_compress_negotiate("lz4"));
	TEST_ASSERT_EQUAL_UINT8(WG_COMPRESS_NONE, wg_compress_negotiate("deflate"));
	TEST_ASSERT_EQUAL_UINT8(WG_COMPRESS_NONE, wg_compress_negotiate(nullptr));
}

void test_compress_destroy_null(void)
{
	wg_compress_destroy(nullptr); /* should not crash */
}
...
```cmake
        ...
        add_library(wg_compress STATIC src/network/compress.c)
        target_include_directories(wg_compress PUBLIC ${CMAKE_SOURCE_DIR}/src)

        wg_add_test(test_compress tests/unit/test_compress.c wg_compress)
```
	...
	while (1) {
		...
		if (bit == 0) {
			...
			for (size_t i = 0; i < length; i++) {
				size_t wi = (ctx->window_pos - (size_t)offset) % WG_LZS_WINDOW_SIZE;
				if (out_pos >= out_size)
					return -ENOSPC;
...
static wg_lzs_ctx_t ctx;

void setUp(void) { wg_lzs_init(&ctx); }
void tearDown(void) {}
...
{
	uint8_t out[16];
	int ret = wg_lzs_compress(&ctx, (const uint8_t *)"", 0, out, sizeof(out));
...
void test_lzs_compress_short(void)
{
	...
	int clen = wg_lzs_compress(&ctx, data, 5, compressed, sizeof(compressed));
...
void test_lzs_roundtrip_short(void)
{
	...
	wg_lzs_ctx_t enc_ctx, dec_ctx;
	wg_lzs_init(&enc_ctx);
	wg_lzs_init(&dec_ctx);

	int clen = wg_lzs_compress(&enc_ctx, data, 13, compressed, sizeof(compressed));
	TEST_ASSERT_GREATER_THAN(0, clen);

	int dlen = wg_lzs_decompress(&dec_ctx, compressed, (size_t)clen,
...
void test_lzs_roundtrip_repeated(void)
{
	...
	int clen = wg_lzs_compress(&enc_ctx, data, sizeof(data),
...
{
	uint8_t out[16];
	TEST_ASSERT_EQUAL_INT(-EINVAL, wg_lzs_compress(nullptr, out, 1, out, 16));
}

void test_lzs_decompress_null(void)
{
	uint8_t out[16];
	TEST_ASSERT_EQUAL_INT(-EINVAL, wg_lzs_decompress(nullptr, out, 1, out, 16));
}
...
void test_lzs_reset(void)
{
	...
	uint8_t out[64];
	wg_lzs_compress(&c, data, 4, out, sizeof(out));
...
Update `wg_compress_init()` case `WG_COMPRESS_LZS` to allocate and init `wg_lzs_ctx_t`.
Update `wg_compress()` and `wg_decompress()` to call `wg_lzs_compress()`/`wg_lzs_decompress()`.
Update `wg_compress_destroy()` to free LZS context.

**Step 5: CMakeLists.txt**

```cmake
        # Sprint 4 — LZS compression
        add_library(wg_compress_lzs STATIC src/network/compress_lzs.c)
        target_include_directories(wg_compress_lzs PUBLIC ${CMAKE_SOURCE_DIR}/src)

        target_link_libraries(wg_compress PUBLIC wg_compress_lzs)
        wg_add_test(test_compress_lzs tests/unit/test_compress_lzs.c wg_compress_lzs)
```
...
**Context:** Channel switching logic. Uses existing `wg_channel_state_t` from dpd.h. Pure state machine — no I/O.
...
[[nodiscard]] wg_compress_type_t wg_dtls_parse_accept_encoding(const char *header);
```
...
**Files:**
- Modify: `src/core/worker.h` (add compress_ctx to wg_connection_t)
...
**Reference:**
- `docs/wolfguard-docs/wolfguard/protocol/cisco-compatibility.md` — DTLS headers, cipher suites
- `docs/wolfguard-docs/openconnect-protocol/protocol/crypto.md` — Crypto protocol details
