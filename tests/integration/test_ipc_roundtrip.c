...
void test_ipc_roundtrip_auth(void)
{
    ...
    if (pid == 0) {
        ...
        if (iog_ipc_unpack_auth_request(buf, (size_t)n, &req) != 0) {
            _exit(2);
        }
        ...
        if (iog_ipc_send(ch.child_fd, resp_buf, (size_t)packed) != 0) {
            _exit(4);
        }
    ...
    ret = iog_ipc_send(ch.parent_fd, buf, (size_t)packed);
    TEST_ASSERT_EQUAL_INT(0, ret);
    ...
    iog_ipc_auth_response_t resp;
    ret = iog_ipc_unpack_auth_response(buf, (size_t)n, &resp);
    TEST_ASSERT_EQUAL_INT(0, ret);
