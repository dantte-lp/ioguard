...
void test_vpn_flow_cstp_data_roundtrip(void)
{
    ...
    ret = rw_conn_data_process_tun(&conn_data, reply, sizeof(reply));
...
void test_vpn_flow_dpd_probe_response(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, rw_conn_timer_init(&timer, &tcfg));
...
void test_vpn_flow_server_shutdown_sends_disconnect(void)
{
    ...
    rw_shutdown_ctx_t sctx;
    TEST_ASSERT_EQUAL_INT(0, rw_shutdown_init(&sctx, worker, 5));
    int drained = rw_shutdown_drain(&sctx);
...
void test_vpn_flow_multiple_clients(void)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        TEST_ASSERT_EQUAL_INT(0, rw_conn_data_init(&cli_data[i], &cfg));
    }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        int ret = rw_conn_data_process_tls(&cli_data[i]);
        TEST_ASSERT_EQUAL_INT(0, ret);
