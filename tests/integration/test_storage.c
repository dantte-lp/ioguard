...
void setUp(void)
{
	snprintf(mdbx_path, sizeof(mdbx_path),
	         "/tmp/test_integration_%d.mdbx", getpid());
	...
	TEST_ASSERT_EQUAL_INT_MESSAGE(0, rc,
	                              "rw_mdbx_check_format failed in setUp");
...
void test_session_lookup_after_create(void)
{
	...
	TEST_ASSERT_EQUAL_MEMORY(sess.session_id, out.session_id,
	                         RW_SESSION_ID_LEN);
	TEST_ASSERT_EQUAL_MEMORY(sess.cookie_hmac, out.cookie_hmac,
	                         sizeof(sess.cookie_hmac));
...
void test_session_delete_and_verify(void)
{
	...
	size_t count = 0;
	rc = rw_sqlite_audit_query_by_username(&sql_ctx, "user119",
	                                       audit_out, 4, &count);
	TEST_ASSERT_EQUAL_INT(0, rc);
...
void test_ban_flow_mdbx_to_sqlite(void)
{
	...
	int rc = rw_sqlite_ban_add(&sql_ctx, "10.20.30.40",
	                           "brute force", 60);
	TEST_ASSERT_EQUAL_INT(0, rc);
...
void test_crash_recovery_mdbx(void)
{
	...
	int rc = rw_mdbx_init(&mdbx_ctx, mdbx_path);
	TEST_ASSERT_EQUAL_INT_MESSAGE(0, rc,
	                              "rw_mdbx_init failed after crash recovery");
