...
static void create_test_fixtures(void)
{
	...
	snprintf(allowed_file, sizeof(allowed_file), "%s/allowed.dat",
		 tmp_dir);
	snprintf(blocked_file, sizeof(blocked_file), "%s/blocked.dat",
		 tmp_dir);
...
void test_landlock_worker_ruleset_build(void)
{
	...
	if (pid == 0) {
		int rc = rw_landlock_apply(RW_LANDLOCK_WORKER,
					   allowed_file, nullptr);
		_exit(rc == 0 ? 0 : 1);
	}
...
void test_landlock_authmod_ruleset_build(void)
{
	...
	if (pid == 0) {
		int rc = rw_landlock_apply(RW_LANDLOCK_AUTHMOD,
					   allowed_file, blocked_file);
		_exit(rc == 0 ? 0 : 1);
	}
