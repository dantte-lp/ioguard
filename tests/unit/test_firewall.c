...
static iog_fw_session_t make_ipv4_session(void)
{
    iog_fw_session_t s = {0};
    s.af = AF_INET;
...
static iog_fw_session_t make_ipv6_session(void)
{
    iog_fw_session_t s = {0};
    s.af = AF_INET6;
...
{
    iog_fw_session_t s = make_ipv4_session();
    char name[IOG_FW_CHAIN_NAME_MAX];
    ...
    /* Null checks */
    ret = iog_fw_chain_name(nullptr, name, sizeof(name));
    TEST_ASSERT_EQUAL_INT(-EINVAL, ret);

    ret = iog_fw_chain_name(&s, nullptr, sizeof(name));
    TEST_ASSERT_EQUAL_INT(-EINVAL, ret);

    ret = iog_fw_chain_name(&s, name, 0);
    TEST_ASSERT_EQUAL_INT(-EINVAL, ret);

    /* Unsupported address family */
    iog_fw_session_t bad = s;
    bad.af = 999;
    ret = iog_fw_chain_name(&bad, name, sizeof(name));
...
void test_firewall_rule_build_ipv4(void)
{
    ...
    int ret = iog_fw_build_create_batch(&s, &buf, &len);
    TEST_ASSERT_EQUAL_INT(0, ret);
...
{
    iog_fw_session_t s = make_ipv6_session();
    void *buf = nullptr;
    size_t len = 0;
...
void test_firewall_batch_build(void)
{
    ...
    ret = iog_fw_build_create_batch(nullptr, &buf, &len);
    TEST_ASSERT_EQUAL_INT(-EINVAL, ret);

    ret = iog_fw_build_create_batch(&s, nullptr, &len);
    TEST_ASSERT_EQUAL_INT(-EINVAL, ret);

    ret = iog_fw_build_create_batch(&s, &buf, nullptr);
    TEST_ASSERT_EQUAL_INT(-EINVAL, ret);
}
...
void test_firewall_chain_create_requires_root(void)
{
    ...
    iog_fw_session_t s = make_ipv4_session();
    int ret = iog_fw_session_create(&s);
...
void test_firewall_cleanup_on_disconnect(void)
{
    ...
    int ret = iog_fw_build_destroy_batch(&s, &buf, &len);
    TEST_ASSERT_EQUAL_INT(0, ret);
    TEST_ASSERT_NOT_NULL(buf);
    TEST_ASSERT_GREATER_THAN(64, (int)len);
    ...
    size_t create_len = 0;
    ret = iog_fw_build_create_batch(&s, &create_buf, &create_len);
    TEST_ASSERT_EQUAL_INT(0, ret);
