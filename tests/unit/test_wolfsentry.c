...
void test_wolfsentry_check_allowed(void)
{
    ...
    rw_ws_result_t res =
        rw_wolfsentry_check_connection(&ctx, AF_INET, &remote, 12345, &local, 443, IPPROTO_TCP);
...
void test_wolfsentry_add_ban_rule(void)
{
    ...
    rw_ws_result_t res =
        rw_wolfsentry_check_connection(&ctx, AF_INET, &remote, 5555, &local, 443, IPPROTO_TCP);
...
void test_wolfsentry_rate_limit(void)
{
    ...
    rw_ws_result_t res =
        rw_wolfsentry_check_connection(&ctx, AF_INET, &remote, 2000, &local, 443, IPPROTO_TCP);
    TEST_ASSERT_EQUAL_INT(RW_WS_REJECT, res);
}
...
void test_wolfsentry_remove_ban(void)
{
    ...
    rw_ws_result_t res =
        rw_wolfsentry_check_connection(&ctx, AF_INET, &remote, 8080, &local, 443, IPPROTO_TCP);
    TEST_ASSERT_EQUAL_INT(RW_WS_REJECT, res);

    rc = rw_wolfsentry_unban_ip(&ctx, AF_INET, &remote);
    TEST_ASSERT_EQUAL_INT(0, rc);

    res = rw_wolfsentry_check_connection(&ctx, AF_INET, &remote, 8080, &local, 443, IPPROTO_TCP);
    TEST_ASSERT_EQUAL_INT(RW_WS_ACCEPT, res);
}
...
void test_wolfsentry_json_config_load(void)
{
    ...
    rw_ws_result_t res =
        rw_wolfsentry_check_connection(&ctx, AF_INET, &remote, 9999, &local, 443, IPPROTO_TCP);
    TEST_ASSERT_EQUAL_INT(RW_WS_ACCEPT, res);
}
...
void test_wolfsentry_get_action_result(void)
{
    ...
    rw_ws_result_t res =
        rw_wolfsentry_check_connection(nullptr, AF_INET, nullptr, 0, nullptr, 0, 0);
...
void test_wolfsentry_connection_event(void)
{
    ...
    rw_ws_result_t res =
        rw_wolfsentry_check_connection(&ctx, AF_INET, &remote, 54321, &local, 443, IPPROTO_TCP);
    TEST_ASSERT_EQUAL_INT(RW_WS_ACCEPT, res);
    ...
    res = rw_wolfsentry_check_connection(&ctx, AF_INET, &remote, 54322, &local, 443, IPPROTO_TCP);
    TEST_ASSERT_EQUAL_INT(RW_WS_REJECT, res);
}
