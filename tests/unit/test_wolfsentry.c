...
static iog_wolfsentry_ctx_t ctx;

void setUp(void)
{
    int rc = iog_wolfsentry_init(&ctx);
    TEST_ASSERT_EQUAL_INT_MESSAGE(0, rc, "iog_wolfsentry_init failed");
}

void tearDown(void)
{
    iog_wolfsentry_close(&ctx);
}
...
void test_wolfsentry_check_allowed(void)
{
    ...
    iog_ws_result_t res =
        iog_wolfsentry_check_connection(&ctx, AF_INET, &remote, 12345, &local, 443, IPPROTO_TCP);

    TEST_ASSERT_EQUAL_INT(IOG_WS_ACCEPT, res);
}
...
void test_wolfsentry_add_ban_rule(void)
{
    ...
    /* Ban the remote IP. */
    int rc = iog_wolfsentry_ban_ip(&ctx, AF_INET, &remote);
    TEST_ASSERT_EQUAL_INT(0, rc);

    /* Now a connection check from that IP should be rejected. */
    iog_ws_result_t res =
        iog_wolfsentry_check_connection(&ctx, AF_INET, &remote, 5555, &local, 443, IPPROTO_TCP);

    TEST_ASSERT_EQUAL_INT(IOG_WS_REJECT, res);
}
...
void test_wolfsentry_rate_limit(void)
{
    ...
    for (int i = 0; i < threshold; i++) {
        iog_ws_result_t res = iog_wolfsentry_check_connection(
            &ctx, AF_INET, &remote, (uint16_t)(1000 + i), &local, 443, IPPROTO_TCP);
        TEST_ASSERT_EQUAL_INT(IOG_WS_ACCEPT, res);
    }
    ...
    iog_ws_result_t res =
        iog_wolfsentry_check_connection(&ctx, AF_INET, &remote, 2000, &local, 443, IPPROTO_TCP);
    TEST_ASSERT_EQUAL_INT(IOG_WS_REJECT, res);
}
...
void test_wolfsentry_remove_ban(void)
{
    ...
    iog_ws_result_t res =
        iog_wolfsentry_check_connection(&ctx, AF_INET, &remote, 8080, &local, 443, IPPROTO_TCP);
    TEST_ASSERT_EQUAL_INT(IOG_WS_REJECT, res);

    rc = iog_wolfsentry_unban_ip(&ctx, AF_INET, &remote);
    TEST_ASSERT_EQUAL_INT(0, rc);

    res = iog_wolfsentry_check_connection(&ctx, AF_INET, &remote, 8080, &local, 443, IPPROTO_TCP);
    TEST_ASSERT_EQUAL_INT(IOG_WS_ACCEPT, res);
}
...
void test_wolfsentry_json_config_load(void)
{
    ...
    int rc = iog_wolfsentry_load_json(&ctx, json_cfg, strlen(json_cfg));
    TEST_ASSERT_EQUAL_INT(0, rc);
    ...
    iog_ws_result_t res =
        iog_wolfsentry_check_connection(&ctx, AF_INET, &remote, 9999, &local, 443, IPPROTO_TCP);
    TEST_ASSERT_EQUAL_INT(IOG_WS_ACCEPT, res);
}
...
void test_wolfsentry_get_action_result(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT8(0, IOG_WS_ACCEPT);
    TEST_ASSERT_EQUAL_UINT8(1, IOG_WS_REJECT);
    TEST_ASSERT_EQUAL_UINT8(2, IOG_WS_ERROR);

    /* Test that nullptr context returns ERROR. */
    iog_ws_result_t res =
        iog_wolfsentry_check_connection(nullptr, AF_INET, nullptr, 0, nullptr, 0, 0);
    TEST_ASSERT_EQUAL_INT(IOG_WS_ERROR, res);
}
...
void test_wolfsentry_connection_event(void)
{
    ...
    iog_ws_result_t res =
        iog_wolfsentry_check_connection(&ctx, AF_INET, &remote, 54321, &local, 443, IPPROTO_TCP);
    TEST_ASSERT_EQUAL_INT(IOG_WS_ACCEPT, res);
    ...
    res = iog_wolfsentry_check_connection(&ctx, AF_INET, &remote, 54322, &local, 443, IPPROTO_TCP);
    TEST_ASSERT_EQUAL_INT(IOG_WS_REJECT, res);
}
