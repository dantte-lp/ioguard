...
{
    iog_dtls_config_t cfg;
    iog_dtls_config_init(&cfg);
...
void test_dtls_config_validate_valid(void)
{
    iog_dtls_config_t cfg;
    iog_dtls_config_init(&cfg);
    TEST_ASSERT_EQUAL_INT(0, iog_dtls_config_validate(&cfg));
}

void test_dtls_config_validate_null(void)
{
    TEST_ASSERT_EQUAL_INT(-EINVAL, iog_dtls_config_validate(nullptr));
}
...
void test_dtls_config_validate_zero_mtu(void)
{
    ...
    iog_dtls_config_init(&cfg);
    cfg.mtu = 0;
    TEST_ASSERT_EQUAL_INT(-EINVAL, iog_dtls_config_validate(&cfg));
}
...
{
    const char *ciphers = iog_dtls_cisco_ciphers();
...
void test_dtls_create_destroy(void)
{
    iog_dtls_config_t cfg;
    iog_dtls_config_init(&cfg);
    iog_dtls_ctx_t *ctx = iog_dtls_create(&cfg);
    /* May be nullptr if wolfSSL not initialized — that's OK for unit test */
    if (ctx) {
        TEST_ASSERT_EQUAL_UINT32(IOG_DTLS_DEFAULT_MTU, iog_dtls_get_mtu(ctx));
        iog_dtls_destroy(ctx);
    } else {
...
{
    iog_dtls_destroy(nullptr); /* should not crash */
}
