...
void test_hex_encode_basic(void)
{
	...
	char hex[16];
	int ret = wg_dtls_hex_encode(data, sizeof(data), hex, sizeof(hex));
...
void test_hex_decode_basic(void)
{
	...
	uint8_t out[4];
	int ret = wg_dtls_hex_decode(hex, strlen(hex), out, sizeof(out));
...
{
	const uint8_t original[WG_DTLS_MASTER_SECRET_LEN] = {
	...
	int ret = wg_dtls_hex_encode(original, sizeof(original),
	                              hex, sizeof(hex));
	TEST_ASSERT_EQUAL_INT(0, ret);
...
void test_hex_decode_invalid_chars(void)
{
	...
	TEST_ASSERT_EQUAL_INT(-EINVAL,
		wg_dtls_hex_decode("ZZZZ", 4, out, sizeof(out)));
}
...
void test_hex_decode_odd_length(void)
{
	uint8_t out[4];
	TEST_ASSERT_EQUAL_INT(-EINVAL,
		wg_dtls_hex_decode("abc", 3, out, sizeof(out)));
}

void test_master_secret_clear(void)
{
	wg_dtls_master_secret_t ms;
	memset(ms.secret, 0xFF, sizeof(ms.secret));
	memcpy(ms.hex, "abcdef", 6);
	ms.valid = true;

	wg_dtls_master_secret_clear(&ms);

	uint8_t zero[WG_DTLS_MASTER_SECRET_LEN] = {0};
