...
void test_conn_data_dpd_request_response(void)
{
    ...
    /* Inject DPD_REQ */
    TEST_ASSERT_GREATER_THAN(0, inject_cstp(RW_CSTP_DPD_REQ, nullptr, 0));
    ...
    int consumed = rw_cstp_decode(tls_buf, (size_t)n, &decoded);
    TEST_ASSERT_GREATER_THAN(0, consumed);
    TEST_ASSERT_EQUAL_INT(RW_CSTP_DPD_RESP, decoded.type);
}
...
void test_conn_data_keepalive_passthrough(void)
{
    ...
    /* Inject KEEPALIVE */
    TEST_ASSERT_GREATER_THAN(0, inject_cstp(RW_CSTP_KEEPALIVE, nullptr, 0));
...
void test_conn_data_compressed_lz4(void)
{
    ...
    rw_compress_destroy(&compress_ctx);
    TEST_ASSERT_EQUAL_INT(0, rw_compress_init(&compress_ctx, RW_COMPRESS_LZ4));
    ...
    int consumed = rw_cstp_decode(tls_buf, (size_t)n, &decoded);
    TEST_ASSERT_GREATER_THAN(0, consumed);
    TEST_ASSERT_EQUAL_INT(RW_CSTP_COMPRESSED, decoded.type);
...
void test_conn_data_multiple_packets_batch(void)
{
    ...
    TEST_ASSERT_GREATER_THAN(0, inject_cstp(RW_CSTP_DATA, p1, sizeof(p1)));
    TEST_ASSERT_GREATER_THAN(0, inject_cstp(RW_CSTP_DATA, p2, sizeof(p2)));
    TEST_ASSERT_GREATER_THAN(0, inject_cstp(RW_CSTP_DATA, p3, sizeof(p3)));
