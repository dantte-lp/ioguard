...
void test_lzs_roundtrip_short(void)
{
    ...
    int dlen =
        rw_lzs_decompress(&dec_ctx, compressed, (size_t)clen, decompressed, sizeof(decompressed));
...
void test_lzs_roundtrip_repeated(void)
{
    ...
    int clen = rw_lzs_compress(&enc_ctx, data, sizeof(data), compressed, sizeof(compressed));
    TEST_ASSERT_GREATER_THAN(0, clen);
