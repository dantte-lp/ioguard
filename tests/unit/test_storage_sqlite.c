...
static iog_sqlite_ctx_t ctx;

void setUp(void)
{
    int rc = iog_sqlite_init(&ctx, ":memory:");
...
static void make_user(iog_user_record_t *u, const char *name)
{
    memset(u, 0, sizeof(*u));
...
void test_sqlite_user_create(void)
{
    iog_user_record_t u;
    make_user(&u, "alice");
    int rc = iog_sqlite_user_create(&ctx, &u);
    TEST_ASSERT_EQUAL_INT(0, rc);
}
...
void test_sqlite_user_lookup_found(void)
{
    ...
    rc = iog_sqlite_user_lookup(&ctx, "bob", &out);
    TEST_ASSERT_EQUAL_INT(0, rc);
...
{
    iog_user_record_t out;
    int rc = iog_sqlite_user_lookup(&ctx, "nonexistent", &out);
...
void test_sqlite_user_duplicate(void)
{
    ...
    rc = iog_sqlite_user_create(&ctx, &u);
...
{
    bool banned = true;
    int rc = iog_sqlite_ban_check(&ctx, "10.0.0.1", &banned);
    TEST_ASSERT_EQUAL_INT(0, rc);
...
{
    int rc = iog_sqlite_ban_add(&ctx, "10.0.0.99", "brute force", 60);
    TEST_ASSERT_EQUAL_INT(0, rc);

    bool banned = false;
    rc = iog_sqlite_ban_check(&ctx, "10.0.0.99", &banned);
    TEST_ASSERT_EQUAL_INT(0, rc);
    TEST_ASSERT_TRUE(banned);

    /* Different IP should not be banned. */
    banned = true;
    rc = iog_sqlite_ban_check(&ctx, "10.0.0.100", &banned);
    TEST_ASSERT_EQUAL_INT(0, rc);
    TEST_ASSERT_FALSE(banned);
}
...
void test_sqlite_user_totp_set_and_lookup(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, iog_sqlite_user_create(&ctx, &u));
...
void test_sqlite_user_totp_clear(void)
{
    ...
    iog_user_record_t out = {0};
    TEST_ASSERT_EQUAL_INT(0, iog_sqlite_user_lookup(&ctx, "clear_user", &out));
    TEST_ASSERT_FALSE(out.totp_enabled);
...
void test_sqlite_user_totp_set_nonexistent_returns_enoent(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(-ENOENT,
                          iog_sqlite_user_totp_set(&ctx, "ghost", secret, sizeof(secret), "[]"));
}

void test_sqlite_user_totp_null_params(void)
{
    TEST_ASSERT_EQUAL_INT(-EINVAL, iog_sqlite_user_totp_set(nullptr, nullptr, nullptr, 0, nullptr));
    TEST_ASSERT_EQUAL_INT(-EINVAL, iog_sqlite_user_totp_clear(nullptr, nullptr));
}
...
void test_sqlite_injection_prevention(void)
{
    ...
    iog_user_record_t out;
    int rc = iog_sqlite_user_lookup(&ctx, evil, &out);
    TEST_ASSERT_EQUAL_INT(-ENOENT, rc);
