...
void setUp(void)
{
    int ret = rw_vault_init_from_key(TEST_KEY, RW_VAULT_KEY_SIZE, &vault);
    TEST_ASSERT_EQUAL_INT(0, ret);
}

void tearDown(void)
{
    rw_vault_destroy(vault);
    vault = nullptr;
}
...
void test_vault_encrypt_decrypt_roundtrip(void)
{
    ...
    uint8_t blob[sizeof(plain) + RW_VAULT_OVERHEAD];
    size_t blob_len = 0;

    int ret = rw_vault_encrypt(vault, plain, sizeof(plain), blob, sizeof(blob), &blob_len);
    TEST_ASSERT_EQUAL_INT(0, ret);
    TEST_ASSERT_EQUAL_UINT(sizeof(plain) + RW_VAULT_OVERHEAD, blob_len);

    uint8_t decrypted[sizeof(plain)];
    size_t dec_len = 0;
    ret = rw_vault_decrypt(vault, blob, blob_len, decrypted, sizeof(decrypted), &dec_len);
    TEST_ASSERT_EQUAL_INT(0, ret);
...
void test_vault_tampered_ciphertext_fails(void)
{
    ...
    blob[RW_VAULT_IV_SIZE] ^= 0xFF;

    uint8_t out[sizeof(plain)];
    size_t out_len = 0;
    ret = rw_vault_decrypt(vault, blob, blob_len, out, sizeof(out), &out_len);
...
void test_vault_different_iv_each_encrypt(void)
{
    ...
    uint8_t blob1[sizeof(plain) + RW_VAULT_OVERHEAD];
    uint8_t blob2[sizeof(plain) + RW_VAULT_OVERHEAD];
    size_t len1 = 0;
    size_t len2 = 0;

    TEST_ASSERT_EQUAL_INT(0, rw_vault_encrypt(vault, plain, sizeof(plain), blob1, sizeof(blob1),
                                              &len1));
    TEST_ASSERT_EQUAL_INT(0, rw_vault_encrypt(vault, plain, sizeof(plain), blob2, sizeof(blob2),
                                              &len2));
    ...
    size_t d1 = 0;
    size_t d2 = 0;
    TEST_ASSERT_EQUAL_INT(0, rw_vault_decrypt(vault, blob1, len1, dec1, sizeof(dec1), &d1));
    TEST_ASSERT_EQUAL_INT(0, rw_vault_decrypt(vault, blob2, len2, dec2, sizeof(dec2), &d2));
...
void test_vault_wrong_key_fails(void)
{
    ...
    uint8_t blob[sizeof(plain) + RW_VAULT_OVERHEAD];
    size_t blob_len = 0;
    TEST_ASSERT_EQUAL_INT(0, rw_vault_encrypt(vault, plain, sizeof(plain), blob, sizeof(blob),
                                              &blob_len));

    /* Create vault with different key */
    uint8_t bad_key[RW_VAULT_KEY_SIZE] = {0xFF};
    rw_vault_t *bad_vault = nullptr;
    TEST_ASSERT_EQUAL_INT(0, rw_vault_init_from_key(bad_key, RW_VAULT_KEY_SIZE, &bad_vault));

    uint8_t out[sizeof(plain)];
    size_t out_len = 0;
    int ret = rw_vault_decrypt(bad_vault, blob, blob_len, out, sizeof(out), &out_len);
    TEST_ASSERT_EQUAL_INT(-EACCES, ret);

    rw_vault_destroy(bad_vault);
}
...
void test_vault_null_params(void)
{
    uint8_t buf[64];
    size_t len = 0;
    TEST_ASSERT_LESS_THAN_INT(0, rw_vault_encrypt(nullptr, buf, 4, buf, 64, &len));
    TEST_ASSERT_LESS_THAN_INT(0, rw_vault_decrypt(nullptr, buf, 32, buf, 32, &len));
}
