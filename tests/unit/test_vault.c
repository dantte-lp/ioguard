...
void test_vault_encrypt_decrypt_roundtrip(void)
{
	...
	int ret = rw_vault_encrypt(vault, plain, sizeof(plain),
	...
	size_t dec_len = 0;
	ret = rw_vault_decrypt(vault, blob, blob_len,
...
void test_vault_different_iv_each_encrypt(void)
{
	...
	size_t d1 = 0;
	size_t d2 = 0;
	TEST_ASSERT_EQUAL_INT(0, rw_vault_decrypt(vault, blob1, len1,
	                                          dec1, sizeof(dec1), &d1));
	TEST_ASSERT_EQUAL_INT(0, rw_vault_decrypt(vault, blob2, len2,
	                                          dec2, sizeof(dec2), &d2));
...
void test_vault_wrong_key_fails(void)
{
	...
	TEST_ASSERT_EQUAL_INT(0, rw_vault_init_from_key(bad_key,
	                                                RW_VAULT_KEY_SIZE,
	                                                &bad_vault));

	uint8_t out[sizeof(plain)];
	size_t out_len = 0;
	int ret = rw_vault_decrypt(bad_vault, blob, blob_len,
	                           out, sizeof(out), &out_len);
	TEST_ASSERT_EQUAL_INT(-EACCES, ret);
...
void test_vault_null_params(void)
{
	uint8_t buf[64];
	size_t len = 0;
	TEST_ASSERT_LESS_THAN_INT(0, rw_vault_encrypt(nullptr, buf, 4,
	                                              buf, 64, &len));
	TEST_ASSERT_LESS_THAN_INT(0, rw_vault_decrypt(nullptr, buf, 32,
	                                              buf, 32, &len));
}
