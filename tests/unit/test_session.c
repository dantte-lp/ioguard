...
void setUp(void)
{
    store = rw_session_store_create(8);
    TEST_ASSERT_NOT_NULL(store);
}

void tearDown(void)
{
    rw_session_store_destroy(store);
    store = nullptr;
}

void test_session_store_create_destroy(void)
{
    rw_session_store_t *s = rw_session_store_create(16);

    TEST_ASSERT_NOT_NULL(s);
    rw_session_store_destroy(s);

    /* nullptr destroy must not crash */
    rw_session_store_destroy(nullptr);
}

void test_session_create_basic(void)
{
    rw_session_t *session = nullptr;
    int ret = rw_session_create(store, "alice", "vpn-users", 3600, &session);
    ...
    uint8_t zeros[RW_SESSION_COOKIE_SIZE] = {0};
    int all_zero = 1;

    for (size_t i = 0; i < RW_SESSION_COOKIE_SIZE; i++) {
...
{
    rw_session_t *created = nullptr;
    int ret = rw_session_create(store, "bob", "admins", 3600, &created);
    ...
    memcpy(cookie, created->cookie, RW_SESSION_COOKIE_SIZE);
...
void test_session_validate_invalid(void)
{
    ...
    (void)rw_session_create(store, "charlie", nullptr, 3600, &session);

    /* Try validating with a bogus cookie */
    uint8_t bogus[RW_SESSION_COOKIE_SIZE];

    memset(bogus, 0xAA, RW_SESSION_COOKIE_SIZE);

    rw_session_t *out = nullptr;
    int ret = rw_session_validate(store, bogus, RW_SESSION_COOKIE_SIZE, &out);
...
{
    rw_session_t *session = nullptr;
    int ret = rw_session_create(store, "dave", "users", 3600, &session);

    TEST_ASSERT_EQUAL_INT(0, ret);

    uint8_t cookie[RW_SESSION_COOKIE_SIZE];

    memcpy(cookie, session->cookie, RW_SESSION_COOKIE_SIZE);
    ...
    ret = rw_session_validate(store, cookie, RW_SESSION_COOKIE_SIZE, &out);
...
{
    rw_session_t *session = nullptr;
    int ret = rw_session_create(store, "eve", nullptr, 1, &session);
    ...
    uint32_t cleaned = rw_session_cleanup_expired(store);

    TEST_ASSERT_EQUAL_UINT32(1, cleaned);
    TEST_ASSERT_EQUAL_UINT32(0, rw_session_count(store));
}
...
void test_session_max_capacity(void)
{
    ...
    for (uint32_t i = 0; i < 8; i++) {
        rw_session_t *session = nullptr;
        int ret = rw_session_create(store, "user", nullptr, 3600, &session);

        TEST_ASSERT_EQUAL_INT(0, ret);
    }

    /* Next create must fail */
    rw_session_t *overflow = nullptr;
    int ret = rw_session_create(store, "overflow", nullptr, 3600, &overflow);
...
void test_session_count(void)
{
    ...
    rw_session_t *s1 = nullptr;
    rw_session_t *s2 = nullptr;

    (void)rw_session_create(store, "user1", nullptr, 3600, &s1);
    TEST_ASSERT_EQUAL_UINT32(1, rw_session_count(store));

    (void)rw_session_create(store, "user2", nullptr, 3600, &s2);
