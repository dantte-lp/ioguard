...
{
    const char *req = "POST /auth HTTP/1.1\r\n"
    ...
    TEST_ASSERT_EQUAL_STRING("<config-auth><auth>user:pass</auth></config>", parser.request.body);
}

void test_http_parse_connect_tunnel(void)
{
    const char *req = "CONNECT /CSCOSSLC/tunnel HTTP/1.1\r\n"
                      "Host: vpn.example.com\r\n"
...
void test_http_incremental_parse(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT32(full.request.header_count, chunked.request.header_count);
}
