...
void setUp(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, rw_conn_data_init(&conn_data, &data_cfg));
...
static int make_timer(rw_conn_timer_t *timer)
{
    rw_conn_timer_config_t cfg = {
        .dpd = &dpd,
        .data = &conn_data,
        .conn_id = 42,
        .dpd_interval_s = 30,
    ...
    return rw_conn_timer_init(timer, &cfg);
}
...
{
    rw_conn_timer_t timer;
    TEST_ASSERT_EQUAL_INT(-EINVAL, rw_conn_timer_init(nullptr, nullptr));
    TEST_ASSERT_EQUAL_INT(-EINVAL, rw_conn_timer_init(&timer, nullptr));

    rw_conn_timer_config_t cfg = {
        .dpd = nullptr,
        .data = &conn_data,
    };
    TEST_ASSERT_EQUAL_INT(-EINVAL, rw_conn_timer_init(&timer, &cfg));
}
...
void test_timer_dpd_dead_callback(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, rw_conn_timer_handle_dpd(&timer));
    /* Drain TLS */
    uint8_t drain[64];
    (void)read(tls_sv[1], drain, sizeof(drain));
...
void test_timer_keepalive_fires(void)
{
    ...
    int ret = rw_conn_timer_handle_keepalive(&timer);
    TEST_ASSERT_EQUAL_INT(0, ret);
...
void test_timer_connection_idle_timeout(void)
{
    ...
    TEST_ASSERT_FALSE(rw_conn_timer_is_idle(&timer, timer.last_activity));

    /* Simulate time passing beyond idle timeout (300s) */
    TEST_ASSERT_TRUE(rw_conn_timer_is_idle(&timer, timer.last_activity + 301));

    /* Exactly at boundary — not idle yet */
    TEST_ASSERT_FALSE(rw_conn_timer_is_idle(&timer, timer.last_activity + 299));
}
...
void test_timer_reschedule_after_activity(void)
{
    ...
    TEST_ASSERT_FALSE(rw_conn_timer_is_idle(&timer, timer.last_activity + 100));
}
