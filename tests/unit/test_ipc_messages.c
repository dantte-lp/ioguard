...
void test_pack_unpack_session_validate(void)
{
    ...
    uint8_t buf[RW_IPC_MAX_MSG_SIZE];
    ssize_t packed = rw_ipc_pack_session_validate(&req, buf, sizeof(buf));
    TEST_ASSERT_GREATER_THAN(0, packed);

    rw_ipc_session_validate_t decoded;
    int ret = rw_ipc_unpack_session_validate(buf, packed, &decoded);
    TEST_ASSERT_EQUAL_INT(0, ret);
    ...
    rw_ipc_free_session_validate(&decoded);
