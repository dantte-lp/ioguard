...
void test_lz4_compress_short(void)
{
    ...
    int clen = rw_lz4_compress(data, sizeof(data) - 1, compressed, sizeof(compressed));
...
void test_lz4_roundtrip_short(void)
{
    ...
    int dlen = rw_lz4_decompress(compressed, (size_t)clen, decompressed, sizeof(decompressed));
...
void test_lz4_roundtrip_repeated(void)
{
    ...
    int clen = rw_lz4_compress(data, sizeof(data), compressed, sizeof(compressed));
    TEST_ASSERT_GREATER_THAN(0, clen);
