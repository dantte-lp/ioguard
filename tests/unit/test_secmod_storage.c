...
void test_secmod_mdbx_session_create_and_lookup(void)
{
    ...
    int ret = rw_mdbx_session_create(ctx.mdbx, &session);
    TEST_ASSERT_EQUAL_INT(0, ret);

    /* Look up the session */
    rw_session_record_t out;
    memset(&out, 0, sizeof(out));
    ret = rw_mdbx_session_lookup(ctx.mdbx, session.session_id, &out);
    TEST_ASSERT_EQUAL_INT(0, ret);
...
void test_secmod_session_delete_cleans_mdbx(void)
{
    ...
    rw_session_record_t session;
    memset(&session, 0, sizeof(session));
    memset(session.session_id, 0xCC, RW_SESSION_ID_LEN);
    ...
    /* Verify gone */
    rw_session_record_t out;
    int ret = rw_mdbx_session_lookup(ctx.mdbx, session.session_id, &out);
...
typedef struct {
    uint8_t ids[16][RW_SESSION_ID_LEN];
    size_t count;
...
void test_secmod_expired_session_cleanup(void)
{
    ...
    rw_session_record_t session;
    memset(&session, 0, sizeof(session));
    memset(session.session_id, 0xDD, RW_SESSION_ID_LEN);
    ...
    /* Verify it exists */
    uint32_t count = 0;
    TEST_ASSERT_EQUAL_INT(0, rw_mdbx_session_count(ctx.mdbx, &count));
    TEST_ASSERT_EQUAL_UINT(1, count);
    ...
    for (size_t i = 0; i < batch.count; i++) {
        TEST_ASSERT_EQUAL_INT(0, rw_mdbx_session_delete(ctx.mdbx, batch.ids[i]));
    }
...
void test_secmod_validate_reads_mdbx(void)
{
    ...
    rw_session_record_t session;
    memset(&session, 0, sizeof(session));
    memset(session.session_id, 0xEE, RW_SESSION_ID_LEN);
