...
{
    iog_dpd_ctx_t ctx;
    iog_dpd_init(&ctx, 0, 0);

    TEST_ASSERT_EQUAL_UINT32(IOG_DPD_DEFAULT_INTERVAL_S, ctx.interval_s);
    TEST_ASSERT_EQUAL_UINT32(30, ctx.interval_s);
    TEST_ASSERT_EQUAL_UINT32(IOG_DPD_DEFAULT_MAX_RETRIES, ctx.max_retries);
...
{
    iog_dpd_ctx_t ctx;
    iog_dpd_init(&ctx, 10, 5);

    TEST_ASSERT_EQUAL_UINT8(IOG_DPD_IDLE, ctx.state);
}

void test_dpd_timeout_idle_to_pending(void)
{
    iog_dpd_ctx_t ctx;
    iog_dpd_init(&ctx, 10, 3);

    iog_dpd_state_t st = iog_dpd_on_timeout(&ctx);

    TEST_ASSERT_EQUAL_UINT8(IOG_DPD_PENDING, st);
    TEST_ASSERT_EQUAL_UINT8(IOG_DPD_PENDING, ctx.state);
...
void test_dpd_response_pending_to_idle(void)
{
    ...
    (void)iog_dpd_on_timeout(&ctx);
    TEST_ASSERT_EQUAL_UINT8(IOG_DPD_PENDING, ctx.state);

    iog_dpd_state_t st = iog_dpd_on_response(&ctx, ctx.sequence);

    TEST_ASSERT_EQUAL_UINT8(IOG_DPD_IDLE, st);
    TEST_ASSERT_EQUAL_UINT8(IOG_DPD_IDLE, ctx.state);
...
void test_dpd_max_retries_to_dead(void)
{
    ...
    st = iog_dpd_on_timeout(&ctx);
    TEST_ASSERT_EQUAL_UINT8(IOG_DPD_DEAD, st);
...
void test_dpd_request_sets_response_flag(void)
{
    ...
    iog_dpd_state_t st = iog_dpd_on_request(&ctx, 42);
...
void test_dpd_channel_initial_cstp_only(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT8(IOG_CHANNEL_CSTP_ONLY, ctx.channel);
}
