...
void test_dpd_init_defaults(void)
{
	...
	TEST_ASSERT_EQUAL_UINT32(WG_DPD_DEFAULT_INTERVAL_S, ctx.interval_s);
	TEST_ASSERT_EQUAL_UINT32(30, ctx.interval_s);
	TEST_ASSERT_EQUAL_UINT32(WG_DPD_DEFAULT_MAX_RETRIES, ctx.max_retries);
...
{
	wg_dpd_ctx_t ctx;
	wg_dpd_init(&ctx, 10, 5);

	TEST_ASSERT_EQUAL_UINT8(WG_DPD_IDLE, ctx.state);
}

void test_dpd_timeout_idle_to_pending(void)
{
	wg_dpd_ctx_t ctx;
	wg_dpd_init(&ctx, 10, 3);

	wg_dpd_state_t st = wg_dpd_on_timeout(&ctx);

	TEST_ASSERT_EQUAL_UINT8(WG_DPD_PENDING, st);
	TEST_ASSERT_EQUAL_UINT8(WG_DPD_PENDING, ctx.state);
...
void test_dpd_response_pending_to_idle(void)
{
	...
	(void)wg_dpd_on_timeout(&ctx);
	TEST_ASSERT_EQUAL_UINT8(WG_DPD_PENDING, ctx.state);

	wg_dpd_state_t st = wg_dpd_on_response(&ctx, ctx.sequence);

	TEST_ASSERT_EQUAL_UINT8(WG_DPD_IDLE, st);
	TEST_ASSERT_EQUAL_UINT8(WG_DPD_IDLE, ctx.state);
...
void test_dpd_max_retries_to_dead(void)
{
	...
	st = wg_dpd_on_timeout(&ctx);
	TEST_ASSERT_EQUAL_UINT8(WG_DPD_DEAD, st);
...
void test_dpd_request_sets_response_flag(void)
{
	...
	wg_dpd_state_t st = wg_dpd_on_request(&ctx, 42);
...
void test_dpd_channel_initial_cstp_only(void)
{
	...
	TEST_ASSERT_EQUAL_UINT8(WG_CHANNEL_CSTP_ONLY, ctx.channel);
}
