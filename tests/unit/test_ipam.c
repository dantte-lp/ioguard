...
static rw_ipam_t ipam;

void setUp(void)
{
    TEST_ASSERT_EQUAL_INT(0, rw_ipam_init(&ipam));
}

void tearDown(void)
{
    rw_ipam_destroy(&ipam);
}
...
{
    TEST_ASSERT_EQUAL_INT(0, rw_ipam_add_pool(&ipam, "10.10.0.0/24"));
...
{
    TEST_ASSERT_EQUAL_INT(0, rw_ipam_add_pool(&ipam, "fd00:abcd::/112"));
    TEST_ASSERT_EQUAL_UINT(1, ipam.pool_count);
...
{
    TEST_ASSERT_EQUAL_INT(-EINVAL, rw_ipam_add_pool(&ipam, "invalid"));
    TEST_ASSERT_EQUAL_INT(-EINVAL, rw_ipam_add_pool(&ipam, ""));
    TEST_ASSERT_EQUAL_INT(-EINVAL, rw_ipam_add_pool(&ipam, "10.0.0.1"));
}

void test_ipam_pool_create_host_addr(void)
{
    /* /32 has no usable host range */
    TEST_ASSERT_EQUAL_INT(-EINVAL, rw_ipam_add_pool(&ipam, "10.0.0.1/32"));
    /* /31 point-to-point — no usable hosts */
    TEST_ASSERT_EQUAL_INT(-EINVAL, rw_ipam_add_pool(&ipam, "10.0.0.0/31"));
}
...
void test_ipam_alloc_ipv4_first(void)
{
    ...
    struct in_addr addr;
    TEST_ASSERT_EQUAL_INT(0, rw_ipam_alloc_ipv4(&ipam, &addr));
...
void test_ipam_alloc_ipv4_sequential(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, rw_ipam_alloc_ipv4(&ipam, &a1));
    TEST_ASSERT_EQUAL_INT(0, rw_ipam_alloc_ipv4(&ipam, &a2));
    TEST_ASSERT_EQUAL_INT(0, rw_ipam_alloc_ipv4(&ipam, &a3));
...
void test_ipam_free_and_reuse(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, rw_ipam_free_ipv4(&ipam, &addr));

    struct in_addr addr2;
    TEST_ASSERT_EQUAL_INT(0, rw_ipam_alloc_ipv4(&ipam, &addr2));
...
void test_ipam_alloc_exhausted(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, rw_ipam_add_pool(&ipam, "10.10.0.0/30"));
    TEST_ASSERT_EQUAL_UINT(2, ipam.pools[0].total_hosts);

    struct in_addr a1, a2, a3;
    TEST_ASSERT_EQUAL_INT(0, rw_ipam_alloc_ipv4(&ipam, &a1));
    TEST_ASSERT_EQUAL_INT(0, rw_ipam_alloc_ipv4(&ipam, &a2));
    TEST_ASSERT_EQUAL_INT(-ENOSPC, rw_ipam_alloc_ipv4(&ipam, &a3));
}
...
void test_ipam_alloc_ipv6(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, rw_ipam_alloc_ipv6(&ipam, &addr));
...
void test_ipam_collision_detect_no_overlap(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, rw_ipam_add_pool(&ipam, "198.51.100.0/24"));
    /* This is TEST-NET-2 (RFC 5737), unlikely to be on any interface */
    int ret = rw_ipam_check_collisions(&ipam);
...
void test_ipam_collision_detect_overlap(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, rw_ipam_add_pool(&ipam, "127.0.0.0/8"));
    TEST_ASSERT_EQUAL_INT(-EEXIST, rw_ipam_check_collisions(&ipam));
}
...
void test_ipam_collision_detect_ipv6(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, rw_ipam_add_pool(&ipam, "::0/120"));
...
{
    TEST_ASSERT_EQUAL_INT(0, rw_ipam_add_pool(&ipam, "10.10.0.0/24"));
    TEST_ASSERT_EQUAL_INT(0, rw_ipam_add_pool(&ipam, "10.20.0.0/24"));
    TEST_ASSERT_EQUAL_UINT(2, ipam.pool_count);
}

void test_ipam_multi_pool_alloc_first(void)
{
    TEST_ASSERT_EQUAL_INT(0, rw_ipam_add_pool(&ipam, "10.10.0.0/30")); /* 2 hosts */
    TEST_ASSERT_EQUAL_INT(0, rw_ipam_add_pool(&ipam, "10.20.0.0/24")); /* 254 hosts */
...
void test_ipam_reserve_specific_ipv4(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, rw_ipam_reserve_ipv4(&ipam, &addr));
...
void test_ipam_reserve_already_taken(void)
{
    ...
    inet_pton(AF_INET, "10.10.0.50", &addr);
    TEST_ASSERT_EQUAL_INT(0, rw_ipam_reserve_ipv4(&ipam, &addr));
    TEST_ASSERT_EQUAL_INT(-EADDRINUSE, rw_ipam_reserve_ipv4(&ipam, &addr));
}
...
void test_ipam_stats_total_and_used(void)
{
    ...
    rw_ipam_stats_t stats;
    rw_ipam_get_stats(&ipam, &stats);
    ...
    /* Free one, recheck */
    TEST_ASSERT_EQUAL_INT(0, rw_ipam_free_ipv4(&ipam, &a1));
    rw_ipam_get_stats(&ipam, &stats);
